...
local bossString = "Hungering Destroyer"      -- not right at all
WGBM[bossString] = {}

WGBM[bossString].Defensive = function(spell, unit, args)
    if args and args[2] <= 60 then
        if WarGod.Unit:GetPlayer():DebuffRemaining("Gluttonous Miasma","HARMFUL") > 0 then
            local timeTillAoe = 1337
            local now = GetTime()
            if WarGod.Unit:GetPlayer():DebuffRemaining("Volatile Ejection", "HARMFUL") > 0 then
                return true
            end
            for msg,time in pairs(WGBM.timers) do
                local timeDiff = time - now
                if timeDiff < timeTillAoe and timeDiff > -2 then
                    if strmatch(msg, "^Expunge") then
                        timeTillAoe = timeDiff
                    elseif strmatch(msg, "^Consume") or UnitCastingInfo("boss1") == "Consume" or UnitChannelInfo("boss1") == "Consume" then
                        timeTillAoe = timeDiff
                    elseif strmatch(msg, "^Desolate") then
                        timeTillAoe = timeDiff
                    end
                end
            end
            if timeTillAoe < 3 then
                return true
            end
        end
    end
...
WGBM[bossString].FriendlyBlacklist = function(spell, unit, args)
    ...
    if unit:DebuffRemaining("Gluttonous Miasma","HARMFUL") > 0 then
        return true
    end
end
...
WGBM[bossString].Taunt = function(spell, unit, args)
    ...
    if unit.name == bossString then
        local remains, stacks = unit:BuffRemaining("Growing Hunger", "HELPFUL")
        if stacks > 6 then
...
WGBM[bossString].DamageCD = function(spell, unit, args)
    ...
    for msg,time in pairs(WGBM.timers) do
        local timeDiff = time - now
        if strmatch(msg, "^Consume") or UnitCastingInfo("boss1") == "Consume" or UnitChannelInfo("boss1") == "Consume" then
            if timeDiff < 15 and (timeDiff > -5 and IsMoving() or timeDiff >= 0) then
                return
            end
        end
    end
