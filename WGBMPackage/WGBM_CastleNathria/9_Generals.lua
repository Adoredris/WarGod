...
local bossString = "Stone Legion Generals"      -- not right at all
WGBM[bossString] = {}


WGBM[bossString].DPSBlacklist = function(spell, unit, args)

    if unit:BuffRemaining("Sinsoul Bulwark", "HELPFUL") > 0 then
        return true
    elseif unit:BuffRemaining("Hardened Stone Form", "HELPFUL") > 0 then
...
WGBM[bossString].Priority = function(spell, unit, args)
    ...
    local score = 10
    if name == "Stone Legion Goliath" then
        score = 25
    elseif name == "Stone Legion Commando" then
        score = 0
    end
    if name ~= "General Kaal" and name ~= "General Grashaal" then
